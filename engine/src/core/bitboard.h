      ...
      public:
        ...
        to_string() const {
            ...
            for (const Square sq : *this) {
                const auto idx = row_len + (RANK_8 - sq.rank()).value() * row_len + col_len * (sq.file().value() + 1);
                assert(idx >= 0);
                out[static_cast<std::size_t>(idx)] = 'X';
            }
        ...
        struct ShiftIndexer {
            ...
            find_magic(const mask_type mask, const std::vector<mask_type>& blockers) {
                ...
                for (uint64_t tries = 0; tries < MAX_TRIES; ++tries) {
                    ...
                    bool             fail      = false;
                    std::vector      used(1ULL << bit::popcount(mask), false);

                    for (const auto& blocker : blockers) {
                        const auto idx =
                            static_cast<index_type>(((blocker & mask) * candidate) >> (64 - bit::popcount(mask)));
                        if (used.at(idx)) {
                            fail = true;
                            break;
                        }
