...
namespace chepp {
    struct TimeManager {
      ...
      private:
        [[nodiscard]] int
        estimate_moves_to_go() const {
            assert(m_constraints.moves_to_go < 0);
            return 30;
            const int baseline = m_params.baseline_moves_to_go;
            int       estimate = baseline;
            estimate -=
                std::min(std::abs(m_init_info.static_eval / 100), 10); // if high eval we will probably finish sooner
