...
struct MoveSelector {
    ...
    enum class Stage { Root, Search, QSearch, ProbCut };
    ...
    MoveSelector(Range&& moves, const Stage stage, const SearchStack::Node& ss, const Params& params,
                 const Move tt_move = Move::none())
        : m_stage(stage), m_ss(ss), m_params(params), m_tt_move(tt_move),
  ...
  private:
    MoveScoreT score_move(const Move m) const {
        ...
        switch (m_stage) {
            ...
            case Stage::Search: {
                if (m == m_ss.killer1) {
                    score += m_params.killer1_bonus;
                    break;
                }
                if (m == m_ss.killer2) {
                    score += m_params.killer2_bonus;
                    break;
                }
