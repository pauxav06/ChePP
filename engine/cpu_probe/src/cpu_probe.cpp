...
int main(int argc, char** argv) {
    ...
    std::map<std::string, std::vector<std::string>> gcc_flags = {
        ...
        {"PEXT",{"-mbmi2"}},
        {"NEON",{"-mfpu=neon"}},
        {"NEON_V8",{"-mfpu=neonv8"}}, {"NEON_DOT",{"-mfpu=neon-dotprod"}},
    ...
    std::map<std::string,bool> cpu_features = {
        {"SSE2", cpuinfo_has_x86_sse2()},
        {"SSE3", cpuinfo_has_x86_sse3()},
        {"AVX2", cpuinfo_has_x86_avx2()},
        {"AVX512", cpuinfo_has_x86_avx512f() && cpuinfo_has_x86_avx512bw() && cpuinfo_has_x86_avx512vl() && cpuinfo_has_x86_avx512dq()},
    ...
    for(auto &f : cpu_features)
        if(f.second && flags.contains(f.first))
            for (const auto &flag : flags[f.first])
                enabled_flags.push_back(flag);
    ...
    for(auto &f : cpu_features)
    {
        ...
        if (f.second) std::cout << "Detected hardware feature : " << f.first << "\n";
        else std::cout << "Unavailablle hardware feature : " << f.first << "\n";
