...
template <> struct scalar_type_traits<ScalarType::UInt8> {
    using type = uint8_t;
...
template <> struct scalar_type_traits<ScalarType::Float32> {
    using type = float;
    static constexpr const char* name        = "float32";
    static constexpr const char* cpp_type    = "float";
    static constexpr const char* literal_suffix = "f";
};
template <> struct scalar_type_traits<ScalarType::Float64> {
    using type = double;
...
#define DEFINE_SCALAR_ENUM_TRAIT(cpp_type, enum_value) \
    template <> struct scalar_enum_traits<cpp_type> {  \
