...
#include "../../../engine/include/ChePP/core/types.h"
...
namespace GrapheusData {
    ...
    struct Position {
        ...
        struct PieceList {
            ...
            static PieceList
            make_piece_list(const chess::Position& pos) {
        ...
        static Position
        from_binpack_entry(const binpack::TrainingDataEntry& entry) {
            PositionMeta meta{};
            meta.m_castling_and_active_player =
                (chess::ordinal(entry.pos.sideToMove()) << 7) | static_cast<uint8_t>(entry.pos.castlingRights());
            ...
            return Position{PieceList::make_piece_list(entry.pos), entry.pos.piecesBB().bits(), meta, res};
        }
    ...
    static Header
    make_header(const std::size_t size) {
        Header header{};
